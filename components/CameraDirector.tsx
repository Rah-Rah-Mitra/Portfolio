...
const CameraDirector: React.FC<{ shot: CameraShotDefinition; onChange: (shot: CameraShotDefinition) => void }> = ({ shot, onChange }) => {
  ...
  const lighting = shot.lighting ?? { key: 3, fill: 2.1, environment: 1 };
  ...
  return <aside className="camera-director" aria-label="Development camera director"><strong>Camera Director · DEV</strong>
    ...
    {number('Key light', lighting.key, (value) => onChange({ ...shot, lighting: { ...lighting, key: value } }), { min: 0 })}{number('Fill light', lighting.fill, (value) => onChange({ ...shot, lighting: { ...lighting, fill: value } }), { min: 0 })}{number('Environment', lighting.environment, (value) => onChange({ ...shot, lighting: { ...lighting, environment: value } }), { min: 0 })}
    ...
    <details><summary>Responsive overrides</summary><textarea rows={5} defaultValue={JSON.stringify(shot.responsive ?? {}, null, 2)} aria-label="Responsive override JSON" onBlur={(event) => { try { onChange({ ...shot, responsive: JSON.parse(event.target.value) }); setMessage('Responsive overrides parsed.'); } catch { setMessage('Responsive override JSON is invalid.'); } }} /></details>
