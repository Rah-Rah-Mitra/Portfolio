...
const OpticalBenchWorld: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const courierUpgrade = startCourierAssetUpgrade({
      ...
      fallback: courier,
      parent: scene,
      getPose: () => controller.getState().reaction?.alias ?? controller.getState().characterPoseId,
    ...
    const unsubscribeCourierPose = controller.subscribe((state) => { courier.setPose(state.reaction?.alias ?? state.characterPoseId); render(); });
