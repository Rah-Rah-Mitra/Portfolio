...
import { EXPLORE_CONTROL_EVENT, PORTFOLIO_WORLD_EVENT, consumePendingExploreControl, dispatchPortfolioWorldEvent, type ExploreControlDetail } from '../lib/worldEvents';
...
import { startCourierAssetUpgrade } from '../world/productionCourierLoader';
...
import { resolveCourierPose } from '../lib/courierReactions';
...
const OpticalBenchWorld: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const disposeObjectGraph = (root: THREE.Object3D) => root.traverse((object) => { if (!(object instanceof THREE.Mesh || object instanceof THREE.LineSegments)) return; object.geometry.dispose(); (Array.isArray(object.material) ? object.material : [object.material]).forEach((material) => material.dispose()); });
    ...
    import('three/examples/jsm/loaders/GLTFLoader.js').then(({ GLTFLoader }) => new GLTFLoader().loadAsync('/workstation/models/optical-rail.glb')).then((gltf) => {
      if (worldDisposed) { disposeObjectGraph(gltf.scene); return; }
      gltf.scene.name = 'workstation-optical-rail';
      gltf.scene.position.set(0, .05, 0);
      gltf.scene.scale.setScalar(.92);
      scene.add(gltf.scene);
      renderer.render(scene, camera);
    }).catch(() => { mount.dataset.mechanismFallback = 'procedural-overlay'; });
    ...
    let courier = createNeutralCourierPlaceholder(); scene.add(courier.root);
    ...
    const courierUpgrade = startCourierAssetUpgrade({
      manifest: COURIER_ASSET_CONTRACT,
      fallback: courier,
      parent: scene,
      getPose: () => resolveCourierPose(controller.getState()),
      importer: async (path): Promise<CourierAsset> => {
        const { GLTFLoader } = await import('three/examples/jsm/loaders/GLTFLoader.js');
        const gltf = await new Promise<Awaited<ReturnType<InstanceType<typeof GLTFLoader>['loadAsync']>>>((resolve, reject) => new GLTFLoader().load(path, resolve, undefined, reject));
        const root = new THREE.Group(); root.name = 'optical-courier-production'; root.add(gltf.scene);
        const mixer = new THREE.AnimationMixer(gltf.scene); let action: THREE.AnimationAction | null = null;
        return {
          root,
          setPose: (poseId) => {
            root.userData.poseId = poseId;
            const normalized = poseId.toLowerCase().replace(/[^a-z0-9]/g, '');
            const clip = gltf.animations.find((candidate) => candidate.name.toLowerCase().replace(/[^a-z0-9]/g, '').includes(normalized));
            action?.stop(); action = clip ? mixer.clipAction(clip) : null; action?.reset().play(); mixer.update(0);
          },
          dispose: () => { mixer.stopAllAction(); root.traverse((object) => { if (!(object instanceof THREE.Mesh)) return; object.geometry.dispose(); (Array.isArray(object.material) ? object.material : [object.material]).forEach((material) => material.dispose()); }); },
        };
      },
      onActivate: (asset) => { courier = asset; render(); },
    });
    const unsubscribeCourierPose = controller.subscribe((state) => { courier.setPose(resolveCourierPose(state)); render(); });
    ...
    const worldEvent = (event: Event) => { const detail = (event as CustomEvent<PortfolioWorldEvent>).detail; controller.dispatch(detail); if (detail.type === 'CAMERA_LAB_UPDATED') applyLab(detail.snapshot); if (detail.type === 'QUALITY_CHANGED' && controller.getState().controlOwner === 'transition') { releasePointerCapture(); arbitrator.reset(); finishReturn(); } else render(); };
    const applyExploreControl = (detail: ExploreControlDetail) => { if (detail.action === 'enter') { controller.enterExplore(detail.sceneId); dispatchPortfolioWorldEvent({ type: 'EXPLORE_ENTERED', sceneId: detail.sceneId, source: 'visitor' }); } else { dispatchPortfolioWorldEvent({ type: 'EXPLORE_EXITED', sceneId: detail.sceneId, source: 'visitor' }); releasePointerCapture(); arbitrator.reset(); controller.exitExplore('exit'); finishReturn(); } };
    const exploreEvent = (event: Event) => { consumePendingExploreControl(); applyExploreControl((event as CustomEvent<ExploreControlDetail>).detail); };
    const emitExploreExit = () => dispatchPortfolioWorldEvent({ type: 'EXPLORE_EXITED', sceneId: controller.getState().exploreSceneId ?? 'camera-laboratory', source: 'visitor' });
    const escape = (event: KeyboardEvent) => { if (event.key === 'Escape' && controller.getState().controlOwner === 'visitor') { emitExploreExit(); releasePointerCapture(); arbitrator.escape(); controller.exitExplore('escape'); finishReturn(); } };
    const leaveOnScroll = () => { if (controller.getState().controlOwner !== 'visitor') return; const bounds = localSurface.getBoundingClientRect(); if (bounds.bottom <= 0 || bounds.top >= innerHeight) { emitExploreExit(); releasePointerCapture(); arbitrator.scrollOut(); controller.exitExplore('scroll'); finishReturn(); } };
    ...
    addEventListener(PORTFOLIO_WORLD_EVENT, worldEvent); addEventListener(EXPLORE_CONTROL_EVENT, exploreEvent); addEventListener(WORLD_POLICY_CHANGE_EVENT, policyChange); addEventListener('keydown', escape); addEventListener('scroll', leaveOnScroll, { passive: true });
    const pendingExplore = consumePendingExploreControl(); if (pendingExplore) applyExploreControl(pendingExplore);
    return () => { worldDisposed = true; liveShotRef.current = () => undefined; removeEventListener(PORTFOLIO_WORLD_EVENT, worldEvent); removeEventListener(EXPLORE_CONTROL_EVENT, exploreEvent); removeEventListener(WORLD_POLICY_CHANGE_EVENT, policyChange); removeEventListener('keydown', escape); removeEventListener('scroll', leaveOnScroll); removeEventListener('pointerup', pointerUp); removeEventListener('pointercancel', pointerUp); localSurface.removeEventListener('pointerdown', pointerDown); localSurface.removeEventListener('pointermove', pointerMove); localSurface.removeEventListener('pointerup', pointerUp); localSurface.removeEventListener('pointercancel', pointerUp); releasePointerCapture(); arbitrator.reset(); cancelTransition(); media.revert(); registry.destroy(); resizeObserver.disconnect(); visibilityObserver.disconnect(); unsubscribeCourierPose(); courierUpgrade.dispose(); disposeObjectGraph(scene); renderer.dispose(); renderer.domElement.remove(); };
  }, [controller]);
