...
const OpticalBenchWorld: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => { liveShotRef.current = () => undefined; removeEventListener(PORTFOLIO_WORLD_EVENT, worldEvent); removeEventListener(EXPLORE_CONTROL_EVENT, exploreEvent); removeEventListener(WORLD_POLICY_CHANGE_EVENT, policyChange); removeEventListener('keydown', escape); removeEventListener('scroll', leaveOnScroll); removeEventListener('pointerup', pointerUp); removeEventListener('pointercancel', pointerUp); localSurface.removeEventListener('pointerdown', pointerDown); localSurface.removeEventListener('pointermove', pointerMove); localSurface.removeEventListener('pointerup', pointerUp); localSurface.removeEventListener('pointercancel', pointerUp); releasePointerCapture(); arbitrator.reset(); cancelTransition(); media.revert(); registry.destroy(); resizeObserver.disconnect(); visibilityObserver.disconnect(); unsubscribeCourierPose(); courierUpgrade.dispose(); scene.traverse((object) => { if (!(object instanceof THREE.Mesh || object instanceof THREE.LineSegments)) return; object.geometry.dispose(); (Array.isArray(object.material) ? object.material : [object.material]).forEach((material) => material.dispose()); }); renderer.dispose(); renderer.domElement.remove(); };
  }, [controller]);
