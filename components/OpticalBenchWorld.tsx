...
import { applyCameraShotToAdapter, applyOpticalGeometryToAdapter, resolveSafePlacement } from '../world/opticalWorldState';
...
const OpticalBenchWorld: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fill = new THREE.HemisphereLight(0xffffff, 0x53605d, 2.1); const key = new THREE.DirectionalLight(0xffffff, 3); key.position.set(4, 7, 6); scene.add(fill, key);
    ...
    const applyImmediateShot = (shot: CameraShotDefinition) => {
      activeShotRef.current = shot; setActiveShot(shot); controller.authorCameraShot(shot.id);
      const adapter = { position: camera.position.toArray() as [number, number, number], target: [0, 0, 0] as [number, number, number], fov: camera.fov, near: camera.near, far: camera.far, roll: camera.rotation.z, exposure: renderer.toneMappingExposure, focusDistance: camera.userData.focusDistance ?? 6, lighting: { key: key.intensity, fill: fill.intensity, environment: scene.environmentIntensity }, renderCount: 0 };
      applyCameraShotToAdapter(shot, adapter); camera.position.fromArray(adapter.position); camera.fov = adapter.fov; camera.near = adapter.near; camera.far = adapter.far; camera.updateProjectionMatrix(); camera.lookAt(...adapter.target); camera.rotation.z = adapter.roll; renderer.toneMappingExposure = adapter.exposure; camera.userData.focusDistance = adapter.focusDistance; key.intensity = adapter.lighting.key; fill.intensity = adapter.lighting.fill; scene.environmentIntensity = adapter.lighting.environment; render();
    };
    liveShotRef.current = applyImmediateShot;
    const applyShot = (shot: CameraShotDefinition, duration = shot.transition.duration) => { activeShotRef.current = shot; setActiveShot(shot); controller.authorCameraShot(shot.id); camera.fov = shot.fov; camera.near = shot.near; camera.far = shot.far; camera.updateProjectionMatrix(); gsap.to(camera.position, { x: shot.position[0], y: shot.position[1], z: shot.position[2], duration, ease: shot.transition.easing, overwrite: true, onUpdate: () => { camera.lookAt(...shot.target); camera.rotation.z = shot.roll ?? 0; render(); } }); };
    applyImmediateShot(activeShotRef.current);

    const registry = new WorldAnchorRegistry({ refresh: () => ScrollTrigger.refresh() }); worldAnchorDefinitions.forEach((definition) => registry.register(definition)); registry.start();
    const placeCourier = (shot: CameraShotDefinition, tier: ResponsiveTier) => { const resolved = registry.resolve(shot, tier).find((anchor) => anchor.chapterId === shot.chapterId); if (!resolved) return; const center = { x: resolved.screenRect.left + resolved.screenRect.width / 2, y: resolved.screenRect.top + resolved.screenRect.height / 2 }; const safe = resolveSafePlacement(center, resolved.safeTextRects, { width: innerWidth, height: innerHeight }); courier.root.visible = safe.visible; courier.root.position.fromArray([resolved.worldPosition[0] + (safe.x - center.x) / 120, resolved.worldPosition[1] - (safe.y - center.y) / 120, resolved.worldPosition[2]]); while (occluderFrames.children.length) occluderFrames.remove(occluderFrames.children[0]); resolved.safeTextRects.forEach((rect) => { const frame = new THREE.Mesh(new THREE.PlaneGeometry(Math.max(.1, rect.width / 180), Math.max(.1, rect.height / 180)), paper); frame.position.set((rect.left + rect.width / 2 - innerWidth / 2) / 180, -(rect.top + rect.height / 2 - innerHeight / 2) / 180, .2); occluderFrames.add(frame); }); };
    ...
    const media = gsap.matchMedia(); media.add({ desktop: '(min-width: 921px)', tablet: '(min-width: 681px) and (max-width: 920px)', mobile: '(max-width: 680px)' }, (context) => { const tier: ResponsiveTier = context.conditions?.mobile ? 'mobile' : context.conditions?.tablet ? 'tablet' : 'desktop'; const responsiveShot = (shot: CameraShotDefinition) => ({ ...shot, ...(tier === 'desktop' ? {} : shot.responsive?.[tier]) }); const triggers = cameraShots.map((shot) => ScrollTrigger.create({ trigger: `#${shot.chapterId}`, start: () => `clamp(${ScrollTrigger.maxScroll(window) * responsiveShot(shot).scrollRange[0]})`, end: () => `clamp(${ScrollTrigger.maxScroll(window) * responsiveShot(shot).scrollRange[1]})`, preventOverlaps: 'portfolio-narrative', fastScrollEnd: 2500, invalidateOnRefresh: true, onUpdate: (self) => { const target = responsiveShot(shot); const result = controller.updateScroll({ chapterId: shot.chapterId, progress: self.progress, velocityPxPerSecond: self.getVelocity(), cameraShotId: target.id }); courier.setPose(result.poseId); if (controller.getState().controlOwner === 'story') applyShot(target, result.durationMs / 1000); }, onEnter: () => { const target = responsiveShot(shot); placeCourier(target, tier); if (controller.getState().controlOwner === 'story') applyShot(target); }, onEnterBack: () => { const target = responsiveShot(shot); placeCourier(target, tier); if (controller.getState().controlOwner === 'story') applyShot(target); } })); return () => triggers.forEach((trigger) => trigger.kill()); });

    let transitionTimer: number | null = null;
    const finishReturn = () => { if (transitionTimer !== null) clearTimeout(transitionTimer); const shot = cameraShots.find((candidate) => candidate.id === controller.getState().cameraShotId) ?? activeShotRef.current; applyShot(shot, .42); transitionTimer = window.setTimeout(() => { controller.completeTransition(); transitionTimer = null; }, 420); };
    const arbitrator = new InteractionArbitrator(); let previousX = 0;
    const pointerDown = (event: PointerEvent) => { if (controller.getState().controlOwner !== 'visitor' || (event.target as Element).closest('button,input,a')) return; previousX = event.clientX; arbitrator.pointerDown({ pointerId: event.pointerId, x: event.clientX, y: event.clientY }, 'camera-laboratory'); };
    localSurface.dataset.worldRotation = '0';
    const pointerMove = (event: PointerEvent) => { if (controller.getState().controlOwner !== 'visitor') return; const intent = arbitrator.pointerMove({ pointerId: event.pointerId, x: event.clientX, y: event.clientY }); if (!intent.preventDefault) return; event.preventDefault(); if (intent.capturePointer && !localSurface.hasPointerCapture(event.pointerId)) localSurface.setPointerCapture(event.pointerId); scene.rotation.y += (event.clientX - previousX) * .004; localSurface.dataset.worldRotation = scene.rotation.y.toFixed(4); previousX = event.clientX; render(); };
    const pointerUp = (event: PointerEvent) => { arbitrator.pointerUp(event.pointerId); if (localSurface.hasPointerCapture(event.pointerId)) localSurface.releasePointerCapture(event.pointerId); };
    ...
    const worldEvent = (event: Event) => { const detail = (event as CustomEvent<PortfolioWorldEvent>).detail; controller.dispatch(detail); if (detail.type === 'CAMERA_LAB_UPDATED') applyLab(detail.snapshot); courier.setPose(controller.getState().reaction?.id ?? controller.getState().characterPoseId); if (detail.type === 'QUALITY_CHANGED' && controller.getState().controlOwner === 'transition') finishReturn(); else render(); };
    const exploreEvent = (event: Event) => { const detail = (event as CustomEvent<{ action: 'enter' | 'exit'; sceneId: 'camera-laboratory' }>).detail; if (detail.action === 'enter') controller.enterExplore(detail.sceneId); else { controller.exitExplore('exit'); finishReturn(); } };
    const escape = (event: KeyboardEvent) => { if (event.key === 'Escape' && controller.getState().controlOwner === 'visitor') { arbitrator.escape(); controller.exitExplore('escape'); finishReturn(); } };
    const leaveOnScroll = () => { if (controller.getState().controlOwner !== 'visitor') return; const bounds = localSurface.getBoundingClientRect(); if (bounds.bottom <= 0 || bounds.top >= innerHeight) { arbitrator.scrollOut(); controller.exitExplore('scroll'); finishReturn(); } };
    addEventListener(worldEventName, worldEvent); addEventListener(exploreEventName, exploreEvent); addEventListener('keydown', escape); addEventListener('scroll', leaveOnScroll, { passive: true });
    return () => { liveShotRef.current = () => undefined; removeEventListener(worldEventName, worldEvent); removeEventListener(exploreEventName, exploreEvent); removeEventListener('keydown', escape); removeEventListener('scroll', leaveOnScroll); localSurface.removeEventListener('pointerdown', pointerDown); localSurface.removeEventListener('pointermove', pointerMove); localSurface.removeEventListener('pointerup', pointerUp); localSurface.removeEventListener('pointercancel', pointerUp); if (transitionTimer !== null) clearTimeout(transitionTimer); media.revert(); registry.destroy(); resizeObserver.disconnect(); visibilityObserver.disconnect(); gsap.killTweensOf(camera.position); courier.dispose(); scene.traverse((object) => { if (!(object instanceof THREE.Mesh || object instanceof THREE.LineSegments)) return; object.geometry.dispose(); (Array.isArray(object.material) ? object.material : [object.material]).forEach((material) => material.dispose()); }); renderer.dispose(); renderer.domElement.remove(); };
  }, [controller]);
